...
Change ID: **G4-RETRY-BACKOFF**
Phase: Reliability G4 (deterministic retry backoff)
Summary: Put a durable, deterministic wait between AUTOMATIC retries of a TRANSIENT failure. `recordTaskFailure` schedules a bounded exponential backoff (15s→30s→60s cap, no jitter) into a new `next_retry_at` for a backoff-eligible + side-effect-safe + within-budget failure and emits one `TASK_RETRY_SCHEDULED`; the manager promote pass skips a not-yet-due task (consuming no attempt) and, once due, does the controlled `failed→queued` (which clears the timer) so the next dispatch increments the attempt once. Not a scheduler/daemon/timer — "wait until due" is a value checked on the next `managerStep`. Does NOT widen the auto-retryable set.
Reason: G1 auto-retry re-queued a transient failure on the very next `managerStep`, so a rate-limited/timed-out provider would be retried immediately and repeatedly. G4 spaces automatic retries deterministically without introducing any background timer.
Files added: `tests/task-backoff.test.ts` (22 tests).
Files modified: `lib/company/manager/failure.ts` (`computeRetryDelayMs` + `RETRY_BACKOFF_BASE_MS`/`RETRY_BACKOFF_MAX_MS` + `BACKOFF_ELIGIBLE_CLASSES`), `lib/company/manager/retry.ts` (schedule `next_retry_at` + emit `TASK_RETRY_SCHEDULED` in `recordTaskFailure`; clear it in `moveFailedToQueued`; due-gate `promoteRetryableFailures`; new `isRetryDue` + `retryTimingForTask`; optional injected `now`), `lib/company/manager/model.ts` (+`TASK_RETRY_SCHEDULED` event), `lib/company/manager/service.ts` (thread optional `now` into the G4 promote pass), `lib/company/model.ts` (`CompanyTask.nextRetryAt`), `lib/db.ts` (`next_retry_at` DDL + additive/idempotent migration + row map/insert), `app/api/company-tasks/[id]/route.ts` (+read-only `retryTiming` on GET), `lib/cli/commands/company.ts` (`task show` next-retry line), `components/MissionsBoard.tsx` (static, non-ticking retry-scheduled badge). Also updated `tests/task-retry.test.ts` E/H to advance the injected clock (behavior legitimately changed — no weakened assertions).
Database: ONE additive nullable column `company_tasks.next_retry_at TEXT` (DDL + guarded `ALTER TABLE`, idempotent). No new table, no lease/scheduler/retry-job table. Older rows load unchanged (NULL = due immediately).
API: additive read-only `retryTiming` (`nextRetryAt`/`retryDue`/`retryAfterMs`, derived at read time) on `GET /api/company-tasks/:id` (failed tasks). No new endpoint; explicit retry endpoint unchanged (uses canonical retry logic).
Behavior: a transient, safe, within-budget failure is scheduled for a bounded backoff and is NOT auto-retried before it is due (no attempt consumed while waiting); at/after due the Manager re-queues it and the next dispatch increments the attempt once. Unsafe/approval/config/gap/unknown/exhausted/interrupted failures get no `next_retry_at`. An operator's explicit Retry may run early but still cannot bypass max_attempts/safety/approval/eligibility/tool-preflight. Reassignment (G3) clears any stale backoff so the new agent queues immediately.
Tests added: 22 (pure policy incl. cap + non-eligible→0; A schedule on transient · B config none · C exhausted none · L unsafe none · M approval none · N unknown none · D/G not-due→no queue/no attempt · E at-due queue · F/I after-due queue + clear · H/J dispatch +1 + completion clears · K reassignment clears/queues-now · S still-eligible stays same-agent · Q safe-interrupted immediate no-backoff · R review-required none · O explicit early retry · P explicit cannot exceed max · T config never loops + timing read model · U one scheduled event · V no re-emit while waiting · W no timers in sources · managerStep manual scenario).
Tests run: `tsc --noEmit`; full `vitest run` (consecutive; backoff file 5/5 deterministic).
Results: typecheck clean; **1760/1760** passing.
Known limits: no background scheduler (retry is checked on the next `managerStep`); no jitter (deterministic tests); backoff applies only to provider/connector-transient classes; no provider/tool fallback, no repeated-failure rotation, no Factory auto-create, no GC. G5.
Rollback: revert the listed files; the `next_retry_at` column is additive + nullable and can be left in place harmlessly (ignored by pre-G4 code).
