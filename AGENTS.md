...
- **Architecture V2 — F6 ✅ Company Intelligence:** a DERIVED, read-only analytical/decision-support layer (`/intelligence`) over F0–F5 — work health, capability coverage, execution, approvals, agent workload + deterministic explainable signals — answering *"what is slowing the company down?"*. **Four projections of one company:** Activity = chronological · Radial = structural · Neural = operational · **Intelligence = analytical**. It never becomes canonical authority, never creates agents (Factory F2 stays sole creator), never auto-remediates, and adds **no new event/telemetry store** (one company-wide read method only, no schema change). **No fake precision** — unavailable metrics are `null` = insufficient_data; NO cost/token/utilization/quality metric, NO "best-employee" ranking. Signals are a CLOSED, deterministic set of 8 types (`stale_task`/`blocked_mission`/`capability_gap`/`single_point_capability`/`approval_delay`/`repeated_failure`/`agent_overload`/`workflow_failure_cluster`) with evidence + thresholds; severity-sorted, deduped. Bounded windows (1h/24h/7d/30d, default 7d; invalid → 400). Privacy-safe (never prompts/tokens/`context_json`/startingInput/outputs/tool args/artifact content). **Navigation only** (deep-links to Missions/G-Brain/Agents/Flows/Approvals); no action/mutation, no `signal → BrainKnowledge`. `lib/company/intelligence/*` + read-only `/api/company/intelligence` + `CompanyIntelligence`/`/intelligence`. See `docs/ARCHITECTURE.md` §6i.
- **Architecture V2 — G-Brain consolidation ✅ (ONE brain + safe cleanup):** `/brain` had a duplicate G-Brain (old `BrainGraphView` `[Radial][Neural]` org constellation ABOVE the new F4/F5 SVG `[Radial][Neural]` workspace) and the newer Radial was BLANK on a bare `/brain`. Consolidated into ONE brain: the original attractive `KnowledgeGraph`/`NeuralGraph` renderers are kept and now consume CANONICAL company data via `lib/brain/company-brain-graph.ts` (STRUCTURAL company→missions→tasks→agents→artifacts/knowledge overview + OPERATIONAL F5 remap) with `lib/brain/kg-ids.ts` reverse-mapping clicks to the ONE Universal Inspector. `BrainWorkspace` is the single CONTROLLER (tab + `GET /api/brain/company-graph` + Inspector + F3 search + `?entity=` FOCUS + operational poll); `/brain` defaults to a bounded company overview (never blank). **Projection ≠ persistence** (a drawn node is not a `brain_entities` row; drawing never ingests knowledge). Retired `BrainGraphView`/`BrainRadial`/`BrainNeural`. **Safe cleanup seam:** `lib/company/cleanup/service.ts` + `/api/missions/:id/{cleanup-preview,archive,delete}` in `MissionsBoard` — Archive (terminal `archived`, hidden-by-default, keeps everything) + Delete-test (scoped, preview→confirm, cascades mission-owned only, retires-not-deletes temporary agents, NEVER touches shared agents/workflows/knowledge; no global wipe). No schema change. See `docs/ARCHITECTURE.md` §6j.
- **Architecture V2 — Tool-backed capability eligibility ✅ (Capability ≠ Tool):** fixed a false-eligibility bug — a capability LABEL (`research.web`) made an agent eligible even without a real live-web/search TOOL, so the factory promoted it, the manager dispatched it, and the task FAILED. Now eligibility for a tool-backed capability requires the capability AND a required, WIRED tool actually on the agent (canonical assignment, never inferred from prose). Generic registry `lib/agents/capability-tools.ts` maps only tool-backed capabilities (`research.web` → web.search/browser.search/web.fetch; none wired → explicit tool gap); model-only caps (analysis/summarization/`research.market`) unchanged. `resolveAgentsForCapabilities` enforces by default (`enforceToolRequirements:false` = capability-only, used by F6). Factory `validateSpecAgainstPolicy` rejects an ungrantable tool-backed capability (fail-safe; +defensive re-check at promotion) — no capability-label-only agent, no auto-grant, no invented connector. Dispatch PREFLIGHT (`delegation.ts`) never starts a tool-deficient agent (even directly-assigned) — task stays queued with `reason: tool_gap`, no `AGENT_STARTED`/agent_run. `getTaskToolGaps` + `/api/company-tasks/:id/eligible-agents` (`{agents, toolGaps}`) + `MissionsBoard` surface the reason. NO schema change; existing agents not mutated. See `docs/ARCHITECTURE.md` §6b + invariant 31.
- **Architecture V2 — Real web/search tool for `research.web` ✅ (follow-up):** `research.web` is now genuinely usable — the tool-backed fix left it correctly unresolved because no web connector existed. Inspection found NO web/search implementation and NO search credential anywhere, so a provider + credential decision was surfaced to the operator (chosen: **Tavily**, **search-only**). New connector `lib/connectors/websearch.ts` (search-only: bounded `{title,url,snippet,source}`; honest — no `TAVILY_API_KEY` ⇒ setup guidance + no network call; provider/timeout errors surfaced, never fabricated) is registered as the ONE canonical tool slug **`web.search`** in `agent-tools.ts` (→ `WIRED_TOOL_SLUGS`), added to `FACTORY_ALLOWED_TOOLS`, and is now the `research.web` requirement (dropped unimplemented `browser.search`/`web.fetch`). Cataloged on Connections + the agent tool picker (`web` kind + lettermark). An agent with `web.search` is eligible and passes dispatch preflight; none has it (or key unset) → honest tool gap. **Search-only = no SSRF surface** (a future `fetchWebPage` must add the full SSRF blocklist first). No fake results, no auto-grant, no invented credential; the resolver/preflight/eligible-agents API were already generic (no change). NO schema change; existing agents not mutated. See `docs/ARCHITECTURE.md` §6b/§6b-web + `docs/PHASE-STATUS.md` "Real web/search tool" + `docs/SECURITY.md`.
- **IGRIS CLI + unified multi-provider model control plane ✅ (this task):** added `igris`, a terminal CONTROL SURFACE that speaks ONLY to the canonical HTTP API (never SQLite, never a parallel engine). Entry `cli/igris.ts` (run via `npm run cli -- …` or the `bin/igris.js` launcher through tsx; `package.json` `bin`); logic in `lib/cli/*` (`client` HTTP-only + injectable fetch, `config` env/`~/.igris` non-secret only, `output` human/`--json` + secret redaction, `router` parse/dispatch, `shell` readline REPL, `commands/*`). Commands: status · models (providers|current|use) · ask · mission (list/show/create/plan/step/archive) · task (list/show/eligible/dispatch) · artifact (show/promote) · brain (search/entity/neighborhood/promote-artifact) · intelligence · flow (list/show/run) · approvals + approval approve/reject. Every mutation previews → confirms (or `--yes`, which still CANNOT bypass Phase-E approval); exit codes 0/1/2/3/4. **Model runtime was already unified** (`lib/models/*` catalog + `routeModel`, one OpenAI-compatible client for OpenAI/Anthropic/Gemini/xAI/DeepSeek/Groq/Mistral/OpenRouter/Together/NVIDIA + Ollama/Custom; AI Gateway + Hermes brains preserved) — this EXTENDED it, not rebuilt: `lib/models/capabilities.ts` (honest capability metadata + `MODEL_CAPABILITY_MISMATCH`), `lib/models/default-model.ts` (canonical `meta`-backed global default + hierarchy `override → agent → default → brain`), and read-only `lib/conductor/ask.ts`. New endpoints: `GET /api/models/providers` (key-free status projection), `GET/POST /api/models/default`, `POST /api/conductor/ask` (read-only, grounded, no silent fallback). NO secret ever leaves the server; NO DB coupling in the CLI; Codex is an OpenAI model via the provider path (no CLI-binary dependency). See `docs/CLI.md`, `docs/ARCHITECTURE.md` §7, invariant 32.
- **Immediate next (do NOT start without instruction):** **Reliability Phase G** (retries, error routes, recorded model fallback, cancellation, workflow recovery, versioning polish) — NOT started; do not begin without instruction. Deferred F6 follow-ons (do NOT start unrequested): optional LLM narrative synthesis over the deterministic snapshot, a thresholds settings UI, cost/token analytics once a telemetry seam exists. Deferred F5 follow-ons (do NOT start unrequested): push/WebSockets, force-directed neural layout, richer approval resolution for direct-agent tasks. Deferred F4 follow-ons: in-place node expansion (vs re-root focus), radial minimap, d3-force layout. Deferred F3 follow-ons: embedding/vector search, legacy-markdown bulk import, knowledge hard-delete. Deferred F2 follow-ons: hard runtime budget metering (agent_runs `cost_usd` seam), per-department factory policies, semantic capability matching, department-manager hierarchy. HRA-2 H4 (remote Hermes) remains an independent option — do NOT make serve the default for new users yet; `getHermesClient(db)` chooses the transport from `meta.hermes_production_transport` (default `acp`), ACP + INC-005 protections stay as rollback. Approval follow-ons NOT yet built (do not start unrequested): approval expiry/TTL (the `expired` status is defined but unused), an auth layer (resolver actor is hard-coded `local_operator`), and Hermes-session approval continuation. See `docs/PHASE-STATUS.md` + `docs/DECISIONS.md` D17–D20.
