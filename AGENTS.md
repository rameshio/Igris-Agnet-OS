...
- **Immediate next (do NOT start without instruction):** **Architecture V2 · F5 — Neural / live operational graph** (animated execution flow, active agents, handoffs; F4's Neural tab is a placeholder and F3's `ProjectionSource` is the interface-only seam — do NOT pipe live `company_events` into the brain viz). Then **F6 — Company Intelligence** (utilization / bottleneck / cost / duplicate-capability analytics — NOT started; F4 shows raw status only). F5/F6 have NOT started; do not begin without instruction. Deferred F4 follow-ons (do NOT start unrequested): in-place node expansion (vs re-root focus), radial minimap, d3-force layout. Deferred F3 follow-ons: embedding/vector search, legacy-markdown bulk import, knowledge hard-delete. Deferred F3 follow-ons (do NOT start unrequested): embedding/vector search, legacy-markdown bulk import, knowledge hard-delete. Deferred F2 follow-ons: hard runtime budget metering (agent_runs `cost_usd` seam), per-department factory policies, semantic capability matching, department-manager hierarchy. HRA-2 H4 (remote Hermes) remains an independent option — do NOT make serve the default for new users yet; `getHermesClient(db)` chooses the transport from `meta.hermes_production_transport` (default `acp`), ACP + INC-005 protections stay as rollback. Approval follow-ons NOT yet built (do not start unrequested): approval expiry/TTL (the `expired` status is defined but unused), an auth layer (resolver actor is hard-coded `local_operator`), and Hermes-session approval continuation. See `docs/PHASE-STATUS.md` + `docs/DECISIONS.md` D17–D20.
