...
- **Immediate next (do NOT start without instruction):** **Architecture V2 · F2 — Agent Factory** (controlled dynamic agent creation to fill capability gaps surfaced by F1 — policy-gated, mission-bound, human-approved; consumes the F1 `CAPABILITY_GAP` result). Then **F3 — G-Brain Core** (entities/knowledge/provenance/explicit memory + artifact ingestion seam). The old "Phase F — Memory-only" plan is **superseded by Architecture V2** (its G-Brain Core work is now V2 F3). F1 Manager/Delegation and F2 Agent Factory have NOT started. HRA-2 H4 (remote Hermes) remains an independent option. Do NOT make serve the default for new users yet. `getHermesClient(db)` chooses the transport from `meta.hermes_production_transport` (default `acp`); ACP + INC-005 protections stay as rollback. Approval follow-ons NOT yet built (do not start unrequested): approval expiry/TTL (the `expired` status is defined but unused), an auth layer (resolver actor is hard-coded `local_operator`), and Hermes-session approval continuation. See `docs/PHASE-STATUS.md` + `docs/DECISIONS.md` D17–D20.
