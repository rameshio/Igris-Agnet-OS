...
- **Immediate next (do NOT start without instruction):** **Architecture V2 · F3 — G-Brain Core** (entities/knowledge/provenance/explicit memory + the artifact ingestion seam — F1/F2 deliberately do NO G-Brain ingestion). The old "Phase F — Memory-only" plan is **superseded by Architecture V2** (its G-Brain Core work is now V2 F3). F3 has NOT started; do not begin without instruction. Deferred F2 follow-ons (do NOT start unrequested): hard runtime budget metering (agent_runs `cost_usd` seam), per-department factory policies, semantic capability matching, department-manager hierarchy. HRA-2 H4 (remote Hermes) remains an independent option — do NOT make serve the default for new users yet; `getHermesClient(db)` chooses the transport from `meta.hermes_production_transport` (default `acp`), ACP + INC-005 protections stay as rollback. Approval follow-ons NOT yet built (do not start unrequested): approval expiry/TTL (the `expired` status is defined but unused), an auth layer (resolver actor is hard-coded `local_operator`), and Hermes-session approval continuation. See `docs/PHASE-STATUS.md` + `docs/DECISIONS.md` D17–D20.
