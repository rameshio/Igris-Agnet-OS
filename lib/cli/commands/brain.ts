...
async function brainEntity(ctx: Ctx): Promise<number> {
  ...
  const res = await ctx.client.get<{ entity?: Record<string, unknown> }>(`/api/brain/entities/${id}`);
  if (!res.ok) return reportApiError(ctx, res.status, res.data);
  const e = (res.data.entity ?? res.data) as Record<string, unknown>;
  emit(ctx.io, ctx.config, kv(Object.entries(e).slice(0, 12).map(([k, v]) => [k, String(v)])), res.data);
  return EXIT.OK;
}
...
async function brainNeighborhood(ctx: Ctx): Promise<number> {
  ...
  const res = await ctx.client.get<{ relationships?: { type?: string; from?: string; to?: string }[] }>(`/api/brain/entities/${id}/relationships`);
  if (!res.ok) return reportApiError(ctx, res.status, res.data);
  const rels = res.data.relationships ?? [];
  const rows = rels.map((r) => [r.type ?? '—', r.from ?? '', r.to ?? '']);
